...
assert(isfolder and makefolder and isfile and writefile, "Executor does not support file system functions")

local _xpcall, _pcall, _task, _math = xpcall, pcall, task, math

-------------------------------------------------------------------------------
-- [MODIFIKASI] SISTEM AUTO-DOWNLOAD MIDI
-------------------------------------------------------------------------------
local folder_name = "MIDI"

-- Buat folder "MIDI" jika belum ada
if not isfolder(folder_name) then
    makefolder(folder_name)
end

-- Masukkan list file MIDI kamu di sini (Wajib pakai Raw URL)
local default_midis = {
    {name = "Summer.mid", url = "https://github.com/Ukrubojvo/api/raw/refs/heads/main/Summer.mid"},
    {name = "Golden-Hour.mid", url = "https://github.com/fahrulrzi/adalah-pokoknya/raw/refs/heads/main/Golden_Hour__JVKE_Updated_Ver.mid"},
    {name = "Kimetsu.mid", url = "https://github.com/fahrulrzi/adalah-pokoknya/raw/refs/heads/main/40946_Kimetsu-no-Yaiba--Gurenge.mid"},
    {name = "Rush-E-Brutal.mid", url = "https://github.com/fahrulrzi/adalah-pokoknya/raw/refs/heads/main/rush_e_real.mid"},
    {name = "Rush-E-Santai.mid", url = "https://github.com/fahrulrzi/adalah-pokoknya/raw/refs/heads/main/Rush%20E.mid"},
    -- Contoh cara nambahin lagu lain:
    -- {name = "LaguBaru.mid", url = "https://raw.githubusercontent.com/username/repo/main/lagu.mid"},
}

-- Looping download
for _, midi in ipairs(default_midis) do
    local file_path = folder_name .. "/" .. midi.name
    
    if not isfile(file_path) then 
        local ok, data = _pcall(function()
            return game:HttpGetAsync(midi.url)
        end)
        
        if ok and data then
            writefile(file_path, data)
        else
            warn("Gagal mendownload: " .. midi.name)
        end
    end
end
-------------------------------------------------------------------------------
